...
if __name__ == "__main__":
    ...
    def objective(trial):
        ...
        args.masks = trial.suggest_int("masks", 1, 10)
        args.samples = trial.suggest_int("samples", 1, 10)
        args.resample_every = trial.suggest_int("resample_every", 5, 50)

        # Hidden layer sizes
        hidden_layers_options = [32, 64, 128, 256, 512, 1024, 2048]
        num_hidden_layers = trial.suggest_int("num_hidden_layers", 1, 9)
        args.hs = [trial.suggest_categorical(f"hs_{i}", hidden_layers_options) for i in range(num_hidden_layers)]
        ...
        roc_auc, balacc, tpr, tnr = utils.test_model(model, val_loader, state_dict, args)
